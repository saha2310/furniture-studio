...
async function syncWorkImages(
  ...
  if (deleteIds.length) {
    const { data: doomed, error } = await supabase
      ...
      .in('id', deleteIds)
      .eq('work_id', workId);
    if (error) return { success: false, message: 'Не удалось подготовить удаление фотографий.' };
    if (doomed?.length) {
      const doomedIds = doomed.map((item) => item.id);
      const { error: deleteError } = await supabase.from('work_images').delete().in('id', doomedIds);
      if (deleteError) return { success: false, message: 'Не удалось удалить выбранные фотографии.' };
  ...
  for (let i = 0; i < replacementFiles.length; i += 1) {
    ...
    if (updateError) {
      ...
      return { success: false, message: 'Не удалось сохранить изменённую фотографию.' };
    }
  ...
  const { data: currentMax, error: maxError } = await supabase
    ...
    .limit(1)
    .maybeSingle();
  if (maxError) return { success: false, message: 'Не удалось определить порядок фотографий.' };
  ...
  if (coverError) return { success: false, message: 'Фотографии сохранены, но не удалось обновить обложку.' };
