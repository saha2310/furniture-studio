...
export async function saveSiteAsset(
  ...
  if (file && file.size > MAX_IMAGE_SIZE_BYTES) return { success: false, message: 'Файл превышает 8 МБ.' };

  const supabase = await createClient();
  const { data: current, error: currentError } = await supabase.from('site_settings').select(field).eq('id', 1).maybeSingle();
  if (currentError) return { success: false, message: 'Не удалось прочитать текущую настройку.' };
