...
async function validateImage(file: File | null): Promise<string | null> {
  ...
  if (file.size > MAX_IMAGE_SIZE_BYTES) return 'Файл превышает 8 МБ.';
  return null;
}
...
export async function createCategory(_prev: ActionResult | null, formData: FormData): Promise<ActionResult> {
  ...
  if (error || !data) {
    const message = error?.code === '23505' ? 'Категория с таким URL уже существует.' : 'Не удалось создать категорию.';
  ...
  if (file) {
    ...
    if (updateError) {
      ...
      await supabase.from('categories').delete().eq('id', data.id);
      return { success: false, message: 'Не удалось сохранить изображение категории.' };
    }
  }
...
export async function updateCategory(categoryId: string, _prev: ActionResult | null, formData: FormData): Promise<ActionResult> {
  ...
  if (currentError || !current) return { success: false, message: 'Категория не найдена.' };
  ...
  if (error) {
    ...
    const message = error.code === '23505' ? 'Категория с таким URL уже существует.' : 'Не удалось сохранить изменения.';
    return { success: false, message };
  }
...
export async function deleteCategory(categoryId: string): Promise<ActionResult> {
  ...
  if (error) return { success: false, message: 'Не удалось удалить категорию' };
