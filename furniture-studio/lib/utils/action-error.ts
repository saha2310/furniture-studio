export function actionError(fallback: string, error: unknown): string {
  ...
  if (code === 'PGRST204' || code === 'PGRST205' || code === '42P01' || /column .* does not exist/i.test(message) || /could not find the table/i.test(message)) {
    return `${fallback} В базе не применена миграция новой версии. Выполните supabase/migrations/0004_sync_schema.sql в SQL Editor Supabase и обновите страницу.`;
  }
