  ...
  return (
    <section className="border-b border-white/10 bg-[#171716]">
      <div className="container-studio grid gap-12 py-24 lg:grid-cols-[.75fr,1.5fr] lg:gap-20">
        ...
        {/*
          Линии между плитками намеренно чуть контрастнее (white/20 вместо
          прежних white/10) — сетка остаётся "слитной" без внешних отступов
          между карточками, но границы между ними теперь читаются на глаз,
          а не сливаются в одно пятно поверх фотографий.
        */}
        <div className="grid border-t border-white/20 sm:grid-cols-2">
          {categories.map((category, index) => (
            <Link
              ...
              className="group relative isolate flex min-h-[250px] overflow-hidden flex-col justify-between border-b border-white/20 p-6 transition-colors hover:bg-white/[0.03] sm:border-r sm:p-8 lg:min-h-[300px]"
            >
              {category.image_path && (
                <Image
                  ...
                  className="object-cover opacity-55 transition-transform duration-700 group-hover:scale-[1.04]"
                />
              )}
