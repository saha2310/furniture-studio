        ...
        <div className="grid border-t border-white/10 sm:grid-cols-2">
          {categories.map((category, index) => (
            <Link
              ...
              className="group flex min-h-[250px] flex-col justify-between border-b border-white/10 p-6 transition-colors hover:bg-white/[0.03] sm:border-r sm:p-8 lg:min-h-[300px]"
            >
              ...
              <div className="relative z-10 flex items-start justify-between gap-6">
                <span className="text-[12px] text-stone">0{index + 1}</span>
                <span className="text-sm text-stone transition-transform duration-500 group-hover:translate-x-2">↗</span>
              </div>
              <div className="relative z-10">
                <h3 className="text-3xl tracking-[-0.03em]">{category.name}</h3>
                <p className="mt-4 max-w-[32ch] text-sm leading-6 text-espresso">
