...
const FALLBACK: HeroContent = {
  title: 'Мебель под пространство.',
  description: 'Создаём мягкую мебель и кровати по индивидуальным размерам и вашим пожеланиям.',
...
export function Hero({ content }: { content?: HeroContent | null }) {
  ...
  return (
    <section className="photo-hero relative min-h-[780px] overflow-hidden border-b border-ink/10 bg-canvas sm:min-h-[820px] lg:min-h-screen">
      {data.imagePath ? (
        <Image
      ...
      <div className="absolute inset-0 bg-[linear-gradient(to_right,rgb(var(--photo-mist)/0.7),rgb(var(--photo-mist)/0.25),rgb(var(--photo-mist)/0.15))]" />

      <div className="light-image-content container-studio relative z-10 flex min-h-[780px] flex-col justify-between pb-9 pt-36 sm:min-h-[820px] lg:min-h-screen lg:pb-12 lg:pt-40">
