...
export function ProcessSection({ content }: { content?: ProcessContent | null }) {
  ...
  return (
    <section className="border-b border-black/10 bg-[#d8d3ca] text-[#272521]">
      <div className="container-studio grid gap-14 py-20 lg:grid-cols-[.7fr,1.8fr] lg:gap-24 lg:py-24">
        <div>
          <p className="text-[10px] uppercase tracking-[0.2em] text-black/45">процесс</p>
          <h2 className="mt-5 max-w-[8ch] text-[clamp(2.7rem,5vw,5.2rem)] leading-[.92] tracking-[-0.04em]">Как мы работаем</h2>
        </div>
        <ol className="grid border-t border-black/15 sm:grid-cols-2 xl:grid-cols-4">
          {steps.map((step, i) => (
            <li key={`${step.title}-${i}`} className="border-b border-black/15 p-6 sm:border-r sm:p-7 xl:min-h-[250px]">
              <div className="flex items-center gap-4 text-[12px] text-black/45">
                <span>{String(i + 1).padStart(2, '0')}</span><span className="h-px flex-1 bg-black/15" />
              </div>
              <h3 className="mt-14 text-[16px] uppercase tracking-[0.04em]">{step.title}</h3>
              <p className="mt-4 text-[13px] leading-6 text-black/55">{step.description}</p>
            </li>
          ))}
        </ol>
      </div>
