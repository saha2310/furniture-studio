...
export function FavoritesContent({ works }: { works: WorkWithUrls[] }) {
  ...
  if (!ready) {
    return <p className="border-y border-white/10 py-24 text-center text-sm text-stone">Загрузка избранного…</p>;
  }

  if (favorites.length === 0) {
    return (
      <div className="flex min-h-[34vh] flex-col items-center justify-center border-y border-white/10 py-20 text-center">
        <NavigationIcon name="favorites" className="h-9 w-9 text-white/70" />
