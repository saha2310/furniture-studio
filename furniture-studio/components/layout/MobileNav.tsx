    ...
    <>
      ...
      >
        <span className="relative block h-4 w-6">
          ...
          <span className={`absolute left-0 top-[7px] h-px w-6 bg-ink transition-opacity duration-300 ${open ? 'opacity-0' : ''}`} />
          <span className={`absolute left-0 top-[14px] h-px w-6 bg-ink transition-transform duration-300 ${open ? '-translate-y-[7px] -rotate-45' : ''}`} />
        </span>
      </button>
        ...
        <nav className="flex h-full flex-col justify-center px-6 pb-20 pt-24 sm:px-10">
          {[...links, { id: 'favorites-mobile', href: '/favorites', label: 'Избранные' }].map((link, i) => (
            <Link
              ...
              className="group flex items-center justify-between border-b border-ink/10 py-5 text-[clamp(2rem,8vw,4rem)] font-light tracking-[-0.035em]"
            >
