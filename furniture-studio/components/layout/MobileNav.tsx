  ...
  return (
    <>
      ...
      <div
        ...
        className={`mobile-nav-panel fixed inset-0 z-[9998] md:hidden transition-[opacity,visibility,background-color] duration-300 ${open ? 'visible opacity-100' : 'invisible opacity-0'}`}
        aria-hidden={!open}
      >
        <nav className="flex h-full flex-col justify-center px-6 pb-20 pt-24 sm:px-10">
          {[...links, { id: 'favorites-mobile', href: '/favorites', label: 'Избранные' }].map((link, i) => (
            <Link
              ...
              className="mobile-nav-item group flex items-center justify-between border-b border-ink/10 py-5 text-[clamp(2rem,8vw,4rem)] font-light tracking-[-0.035em]"
            >
