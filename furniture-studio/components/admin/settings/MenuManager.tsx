...
export function MenuManager({ items }: { items: Item[] }) {
  const [createState, createAction] = useFormState(createMenuItem, null);
  return <div className="space-y-3">
    <form action={createAction} className="grid gap-3 border border-white/10 bg-[#1b1a18] p-4 sm:grid-cols-[1fr,1fr,100px,auto,auto] sm:items-end">
      ...
      <label className="flex items-center gap-2 pb-2 text-xs text-espresso"><input name="is_visible" type="checkbox" defaultChecked /> Показывать</label><Submit label="Добавить" />{createState && <span className="text-xs text-espresso sm:col-span-full">{createState.message}</span>}
    </form>
