  ...
  return (
    <tr className="border-b border-stone/40 align-top last:border-0">
      <td className="px-4 py-3">
        <Link href={`/admin/contacts/${request.id}`} className="block hover:text-white"><p>{request.name}</p>
        <p className="text-sm text-espresso">{request.contact}</p></Link>
      </td>
      <td className="px-4 py-3 text-espresso">{request.request_type || '—'}</td>
      <td className="max-w-xs px-4 py-3 text-espresso"><Link href={`/admin/contacts/${request.id}`} className="line-clamp-2 hover:text-white">{request.comment || '—'}</Link></td>
