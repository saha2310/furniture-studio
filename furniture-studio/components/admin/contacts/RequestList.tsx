...
function RowActions({ request }: { request: ContactRequest }) {
  return <div className="flex min-w-[190px] items-center justify-end gap-4 whitespace-nowrap">
    <Link href={`/admin/contacts/${request.id}`} className="inline-flex min-h-9 shrink-0 items-center rounded-full border border-ink/20 bg-ink/[0.05] px-3.5 text-[10px] font-medium uppercase tracking-[0.12em] text-ink transition-colors hover:border-ink/35 hover:bg-ink/[0.09] hover:text-ink">Открыть</Link>
    <ConfirmDialog triggerLabel="Удалить" title="Удалить заявку?" description="Это действие необратимо." onConfirm={() => deleteContactRequest(request.id)} />
  </div>;
}
