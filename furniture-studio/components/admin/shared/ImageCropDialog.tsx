        ...
        <div className="grid min-h-0 flex-1 gap-5 overflow-auto p-5 lg:grid-cols-[minmax(0,1fr),280px] sm:p-6">
          ...
          <div className="flex flex-col gap-5">
              ...
              <div className="mt-3 grid grid-cols-2 gap-2">
                {IMAGE_RATIOS.map((item) => (
                  <button
                    ...
                    className={`border px-3 py-2 text-xs transition-colors ${ratio === item.value ? 'border-ink bg-ink text-canvas' : 'border-ink/15 text-ink/70 hover:border-ink/40 hover:text-ink'}`}
                  >
                    {item.label}
                  </button>
                ))}
            ...
            <div className="mt-auto grid gap-2 sm:grid-cols-2 lg:grid-cols-1">
              ...
              <button type="button" onClick={apply} disabled={applying || !image} className="bg-ink px-4 py-3 text-xs uppercase tracking-[0.12em] text-canvas disabled:opacity-50">{applying ? 'Подготавливаем…' : 'Применить обрезку'}</button>
            </div>
          </div>
        </div>
      </div>
