  ...
  return (
    <div className="border border-ink/10 bg-surface p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0"><div className="flex items-center gap-2"><p className="text-sm text-ink/90">{label}</p>{help && <button type="button" onClick={() => setShowHelp((value) => !value)} aria-expanded={showHelp} className="flex h-5 w-5 items-center justify-center border border-ink/20 text-[10px] text-ink/60 hover:border-ink/45 hover:text-ink" aria-label={`Что такое: ${label}`}>i</button>}</div>{help && showHelp && <p className="mt-2 max-w-[42ch] text-xs leading-5 text-ink/55">{help}</p>}</div>
