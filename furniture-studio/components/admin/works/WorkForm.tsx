...
interface WorkFormProps {
  ...
  // Все остальные товары (любая категория, любая группа цветов) — кандидаты
  // для «+ Существующий товар». Только для редактирования: при создании
  // нового товара прикреплять пока нечего.
...
//
// Ограничение «только товары с общей категорией» снято по запросу —
// привязать цветовым вариантом можно любой товар из любой группы/категории,
// подтверждение действия берёт на себя ConfirmDialog у каждой кнопки ниже.
function AttachExistingModal({ groupId, candidates, categories, onClose }: { groupId: string; candidates: WorkWithUrls[]; categories: Category[]; onClose: () => void }) {
  ...
  // Возвращает ActionResult напрямую — так функцию можно передать прямо в
  // ConfirmDialog.onConfirm: он сам покажет "Сохраняем…"/ошибку и закроется
  // при успехе, поэтому здесь достаточно обновить страницу и закрыть модалку.
      ...
      <div role="dialog" aria-modal="true" aria-labelledby={titleId} className="flex max-h-[85vh] w-full max-w-lg flex-col border border-ink/10 bg-surface shadow-2xl">
        <div className="flex items-center justify-between gap-4 border-b border-ink/10 p-5">
          <div>
            ...
            <p className="mt-1 text-xs leading-5 text-ink/40">Показаны все работы, независимо от категории или текущей группы цветов. Перед привязкой или откреплением попросим подтвердить действие.</p>
          </div>
          <button type="button" onClick={onClose} className="shrink-0 text-2xl leading-none text-ink/50 hover:text-ink" aria-label="Закрыть">×</button>
        </div>
                  ...
                  <li key={c.id}>
                    <div className="flex w-full items-center gap-3 px-3 py-2 transition hover:bg-ink/5">
                      ...
                      <span className="shrink-0">
                        {isCurrentGroup ? (
                          <span className="max-w-[150px] text-right text-[9px] uppercase tracking-[0.08em] text-ink/35">Уже в этой группе</span>
                        ) : isOtherGroup ? (
                          <ConfirmDialog
                            triggerLabel="Открепить и привязать"
                            title="Открепить товар от текущей группы и привязать сюда?"
                            description={`Товар «${c.title}» откреплён от своей нынешней группы цветов и станет цветовым вариантом текущего товара. Категория при этом не проверяется — можно привязать товар из любой группы.`}
                            confirmLabel="Открепить и привязать"
                            tone="neutral"
                            onConfirm={() => handleAttach(c.id, true)}
                            triggerClassName="border border-ink/15 px-2.5 py-1.5 text-[9px] uppercase tracking-[0.08em] text-ink/65 hover:border-ink/40 hover:text-ink"
                          />
                        ) : (
                          <ConfirmDialog
                            triggerLabel="Привязать"
                            title="Привязать этот товар как цвет?"
                            description={`Товар «${c.title}» станет цветовым вариантом текущей группы и перестанет быть отдельной карточкой в каталоге. Категория при этом не проверяется — можно привязать товар из любой группы.`}
                            confirmLabel="Привязать"
                            tone="neutral"
                            onConfirm={() => handleAttach(c.id)}
                            triggerClassName="border border-ink/15 px-2.5 py-1.5 text-[9px] uppercase tracking-[0.08em] text-ink/65 hover:border-ink/40 hover:text-ink"
                          />
                        )}
                      </span>
...
function VariantBar({
  ...
  return (
    <div className="flex min-w-0 items-center gap-3 border-b border-ink/10 pb-3">
      ...
      {/* overscroll-x-contain — на мобильных WebKit-браузерах свайп по этой
          горизонтально скроллящейся строке иначе может "перетекать" в
          горизонтальный скролл всей страницы (rubber-band эффект), что и
          ломало sticky-меню выше по странице. Оставляем как подстраховку
          даже после перевода меню на fixed. */}
...
export function WorkForm({ categories, initialData, action, submitLabel, redirectToDetailOnSuccess, colorVariants = [], groupId, prefillTitle, prefillCategoryId, attachCandidates = [], usedColors = [] }: WorkFormProps) {
  ...
  // Подтверждение перед сохранением — форма отправляется через нативный
  // submit (useFormState/Server Action), поэтому подтверждение встроено в
  // сам обработчик onSubmit: первый submit перехватывается и открывает
  // модалку, а реальная отправка происходит через formRef.requestSubmit()
  // из кнопки «Сохранить» в модалке (skipConfirmRef пропускает повторную
  // проверку у этого самого вызова).
  ...
  return (
    <form
      ...
      onSubmit={(event) => {
        ...
        // Подтверждение сохранения: пропускаем эту проверку только для
        // вызова, который сам пришёл из кнопки «Сохранить» модалки ниже.
    ...
    >
      ...
      {confirmSaveOpen && (
        <div
          ...
          onMouseDown={(event) => { if (event.target === event.currentTarget) setConfirmSaveOpen(false); }}
        >
          <div className="w-full max-w-md border border-ink/10 bg-surface p-6 shadow-2xl sm:p-7">
            <h2 className="text-lg text-ink">Сохранить изменения?</h2>
            <p className="mt-2 text-sm leading-6 text-ink/55">Работа «{title || 'без названия'}» будет сохранена с текущими полями, цветом и привязками вариантов. Проверьте данные перед подтверждением.</p>
            <div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
              <button type="button" onClick={() => setConfirmSaveOpen(false)} className="min-h-11 border border-ink/10 px-4 py-3 text-xs uppercase tracking-[0.12em] text-ink/55 hover:text-ink">Отмена</button>
              <button
                type="button"
                onClick={() => { setConfirmSaveOpen(false); skipConfirmRef.current = true; formRef.current?.requestSubmit(); }}
                className="min-h-11 border border-ink/15 bg-ink px-4 py-3 text-xs uppercase tracking-[0.12em] text-canvas hover:bg-espresso"
              >
                Сохранить
              </button>
            </div>
