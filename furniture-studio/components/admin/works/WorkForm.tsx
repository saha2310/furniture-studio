...
export function WorkForm({ categories, initialData, action, submitLabel, redirectToDetailOnSuccess }: WorkFormProps) {
  ...
  useEffect(() => {
    if (state?.success && redirectToDetailOnSuccess && state.id) {
      router.push(`/admin/works/${state.id}`);
    }
  ...
  return (
    <form action={formAction} className="flex flex-col gap-6">
      ...
      <div className="grid gap-6 sm:grid-cols-[1fr,260px]">
        <Textarea name="description" label="Описание" rows={5} defaultValue={initialData?.description ?? ''} />
        <div>
          <p className="text-sm text-espresso">Цена</p>
          <div className="mt-2 flex border border-stone bg-canvas p-1">
            <button type="button" onClick={() => setPriceMode('fixed')} className={`flex-1 px-3 py-2 text-xs ${priceMode === 'fixed' ? 'bg-ink text-canvas' : 'text-espresso'}`}>Указать цену</button>
            <button type="button" onClick={() => setPriceMode('negotiable')} className={`flex-1 px-3 py-2 text-xs ${priceMode === 'negotiable' ? 'bg-ink text-canvas' : 'text-espresso'}`}>По договорённости</button>
          </div>
          <Input name="price" label="" placeholder="Например, 125 000 ₽" defaultValue={priceMode === 'fixed' ? (initialData?.price && initialData.price !== 'По договорённости' ? initialData.price : '') : ''} disabled={priceMode !== 'fixed'} />
          {priceMode === 'negotiable' && <input type="hidden" name="price_mode" value="negotiable" />}
          {priceMode === 'fixed' && <input type="hidden" name="price_mode" value="fixed" />}
        </div>
      </div>
      ...
      <div className="flex items-center gap-4">
        ...
        {state && state.success && !redirectToDetailOnSuccess && (
