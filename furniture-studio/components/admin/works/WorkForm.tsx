...
export function WorkForm({ categories, initialData, action, submitLabel, redirectToDetailOnSuccess }: WorkFormProps) {
  ...
  return (
    <form action={formAction} onChange={() => setDirty(true)} className="max-w-6xl pb-4">
        ...
        <div className="space-y-4">
          ...
          <AdminSection title="Цена" description="Укажите стоимость или выберите вариант «По договорённости»." defaultOpen>
            <div className="grid gap-3 sm:grid-cols-2">
              <label className={`flex min-h-12 cursor-pointer items-center gap-3 border px-4 py-3 text-sm ${priceMode === 'fixed' ? 'border-white/30 bg-white/[0.04] text-white' : 'border-white/10 text-white/55'}`}>
                <input type="radio" name="price_mode_picker" checked={priceMode === 'fixed'} onChange={() => setPriceMode('fixed')} className="accent-white" /> Указать цену
              </label>
              <label className={`flex min-h-12 cursor-pointer items-center gap-3 border px-4 py-3 text-sm ${priceMode === 'negotiable' ? 'border-white/30 bg-white/[0.04] text-white' : 'border-white/10 text-white/55'}`}>
                <input type="radio" name="price_mode_picker" checked={priceMode === 'negotiable'} onChange={() => setPriceMode('negotiable')} className="accent-white" /> По договорённости
              </label>
            </div>
            ...
            <div className="space-y-2">
              {specs.map((spec, index) => (
                <div key={index} className="grid gap-2 sm:grid-cols-[minmax(0,.7fr),minmax(0,1.3fr),44px]">
                  <input name="spec_key" placeholder="Размеры" value={spec.key} onChange={(e) => setSpecs((items) => items.map((item, i) => i === index ? { ...item, key: e.target.value } : item))} className="h-11 border border-white/10 bg-[#111110] px-3 text-sm text-white placeholder:text-white/25 focus:border-white/30" />
                  <input name="spec_value" placeholder="220 × 95 × 85 см" value={spec.value} onChange={(e) => setSpecs((items) => items.map((item, i) => i === index ? { ...item, value: e.target.value } : item))} className="h-11 border border-white/10 bg-[#111110] px-3 text-sm text-white placeholder:text-white/25 focus:border-white/30" />
                  <button type="button" onClick={() => setSpecs((items) => items.filter((_, i) => i !== index))} className="h-11 border border-white/10 text-white/45 hover:border-red-300/30 hover:text-red-200" aria-label={`Удалить характеристику ${index + 1}`}>×</button>
                </div>
              ))}
              <button type="button" onClick={() => setSpecs((items) => [...items, { key: '', value: '' }])} className="min-h-10 text-xs uppercase tracking-[0.12em] text-white/60 hover:text-white">+ Добавить характеристику</button>
            </div>
          </AdminSection>
        </div>

        <aside className="space-y-4 lg:sticky lg:top-5 lg:self-start">
          <section className="border border-white/10 bg-[#171716] p-5">
            <p className="text-[10px] uppercase tracking-[0.16em] text-white/35">Публикация</p>
            <label className="mt-4 flex items-start gap-3 text-sm text-white/80"><input type="checkbox" name="is_featured" defaultChecked={initialData?.is_featured} className="mt-1 h-4 w-4" /><span><span className="block text-white">На главной</span><span className="mt-1 block text-xs leading-5 text-white/40">Показывать в разделе избранных работ.</span></span></label>
      ...
      {dirty && <p className="mt-3 text-xs text-white/35">Есть несохранённые изменения.</p>}
    </form>
  );
}
