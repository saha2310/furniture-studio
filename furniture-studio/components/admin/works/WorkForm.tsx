    ...
    >
      ...
      <div className="mb-5 flex flex-col gap-4 border-b border-ink/10 pb-5 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <SaveButton label={submitLabel} busy={imagesUploading} />
        </div>
      </div>
      ...
      <div className="mt-8 border-t border-ink/10 pt-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div>{state && <FormStatus state={{ status: state.success ? 'success' : 'error', message: state.message }} />}{imagesUploading && <p className="mt-2 text-xs text-ink/45">Дождитесь загрузки фотографий, затем нажмите «Сохранить».</p>}{!imagesUploading && dirty && !state?.success && <p className="mt-2 text-xs text-ink/35">Есть несохранённые изменения.</p>}</div><div className="sm:hidden"><SaveButton label={submitLabel} busy={imagesUploading} /></div></div></div>
      <div className="fixed inset-x-0 bottom-0 z-40 hidden border-t border-ink/10 bg-canvas/90 px-4 py-3 backdrop-blur md:block"><div className="mx-auto flex max-w-[1440px] items-center justify-between gap-4"><p className="text-xs text-ink/45">{imagesUploading ? 'Загружаем фотографии…' : dirty ? 'Изменения ещё не сохранены' : 'Все изменения сохранены'}</p><div className="flex items-center gap-2"><Link href="/admin/works" className="h-11 px-4 text-[9px] uppercase tracking-[0.14em] text-ink/50 hover:text-ink">Отмена</Link><SaveButton label={submitLabel} busy={imagesUploading} /></div></div></div>
    </form>
  );
}
