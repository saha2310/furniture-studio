...
function SubmitButton() { const { pending } = useFormStatus(); return <button type="submit" disabled={pending} className="bg-ink px-6 py-3 text-[10px] uppercase tracking-[0.12em] text-canvas disabled:opacity-50">{pending ? 'Сохраняем…' : 'Сохранить первый экран'}</button>; }
...
export function HeroEditor({ content, isVisible }: { content: HeroContent | null; isVisible: boolean }) {
  ...
  return <form action={formAction} className="border border-ink/10 bg-surface p-5 sm:p-6">
    <div className="flex items-center justify-between"><div><p className="eyebrow">главная</p><h2 className="mt-1 text-lg text-ink">Первый экран</h2></div><label className="flex items-center gap-2 text-xs text-ink/70"><input type="checkbox" name="is_visible" defaultChecked={isVisible} className="h-4 w-4" /> Показывать секцию</label></div>
