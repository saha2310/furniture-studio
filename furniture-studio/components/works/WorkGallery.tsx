  ...
  return (
    <div>
        ...
        {images.length > 1 && (
          <>
            <div className="absolute left-4 top-1/2 z-10 -translate-y-1/2">
              <button type="button" aria-label="Предыдущее изображение" onClick={prev} className="liquid-glass-icon-button liquid-glass-icon-button-lg liquid-glass-on-image opacity-0 transition-opacity duration-300 group-hover:opacity-100">
            ...
            <div className="absolute right-4 top-1/2 z-10 -translate-y-1/2">
              <button type="button" aria-label="Следующее изображение" onClick={next} className="liquid-glass-icon-button liquid-glass-icon-button-lg liquid-glass-on-image opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
      ...
      {lightbox && <div className="fixed inset-0 z-[120] bg-black/80 backdrop-blur-[2px] transition-opacity duration-300" role="dialog" aria-modal="true" aria-label={`Просмотр фотографий: ${title}`}>
        ...
        {images.length > 1 && (
          <button type="button" onClick={prev} aria-label="Предыдущее фото" className="absolute left-1 top-1/2 z-30 -translate-y-1/2 p-5 sm:left-4" style={{ '--glass-text': '255 255 255', '--glass-text-strong': '255 255 255', '--glass-border': '255 255 255' } as React.CSSProperties}>
            <span className="liquid-glass-icon-button liquid-glass-icon-button-sm flex items-center justify-center opacity-35 transition-opacity duration-300 hover:opacity-90 focus-visible:opacity-90">
        ...
        {images.length > 1 && (
          <button type="button" onClick={next} aria-label="Следующее фото" className="absolute right-1 top-1/2 z-30 -translate-y-1/2 p-5 sm:right-4" style={{ '--glass-text': '255 255 255', '--glass-text-strong': '255 255 255', '--glass-border': '255 255 255' } as React.CSSProperties}>
            <span className="liquid-glass-icon-button liquid-glass-icon-button-sm flex items-center justify-center opacity-35 transition-opacity duration-300 hover:opacity-90 focus-visible:opacity-90">
              <svg width="18" height="18" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                <path d="M8 4L14 10L8 16" />
              </svg>
