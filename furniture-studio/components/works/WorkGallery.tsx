...
export function WorkGallery({ images, title }: { images: WorkImageWithUrl[]; title: string }) {
  ...
  return (
    <div>
      <div className="group relative min-w-0 overflow-hidden bg-[#25231f]">
      ...
      {lightbox && <div className="fixed inset-0 z-[120] bg-black/96" role="dialog" aria-modal="true" aria-label={`Просмотр фотографий: ${title}`}>
        ...
        <div className="fixed inset-0 flex h-full w-full touch-none items-center justify-center overflow-hidden px-4 py-16 sm:px-8 sm:py-14" onDoubleClick={() => zoomTo(scale > 1 ? 1 : 2)} onWheel={onWheel} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerCancel={onPointerUp}>
          <img src={images[activeIndex].url} alt={images[activeIndex].alt_text || title} draggable={false} className="max-h-[calc(100vh-7rem)] max-w-[calc(100vw-2rem)] select-none object-contain sm:max-h-[calc(100vh-6rem)] sm:max-w-[calc(100vw-4rem)]" style={{ transform: `translate(${position.x}px, ${position.y}px) scale(${scale})`, transition: drag.current ? 'none' : 'transform 180ms ease-out', cursor: scale > 1 ? 'grab' : 'zoom-in' }} />
        </div>
