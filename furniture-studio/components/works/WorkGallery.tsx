  ...
  return (
    <div>
      <div className="group relative min-w-0 overflow-hidden bg-surface">
        ...
        {images.length > 1 && (
          <>
            <button type="button" aria-label="Предыдущее изображение" onClick={prev} className="gallery-arrow gallery-arrow-prev">
              <svg width="22" height="22" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.25" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                <path d="M12 4L6 10L12 16" />
              </svg>
            </button>
            <button type="button" aria-label="Следующее изображение" onClick={next} className="gallery-arrow gallery-arrow-next">
              <svg width="22" height="22" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.25" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
        ...
        {images.length > 1 && (
          <div className="pointer-events-none absolute inset-x-5 bottom-4 z-10 flex items-center gap-3 sm:inset-x-7 sm:bottom-5">
            <span className="gallery-counter-index tabular-nums">{String(activeIndex + 1).padStart(2, '0')}</span>
            <span className="gallery-progress-track relative h-px flex-1 overflow-hidden">
              <span className="gallery-progress-fill absolute inset-y-0 left-0" style={{ width: `${((activeIndex + 1) / images.length) * 100}%` }} />
            </span>
            <span className="gallery-counter-index tabular-nums">{String(images.length).padStart(2, '0')}</span>
          </div>
        )}
      </div>
