...
export function WorkCard({ work, priority = false }: { work: WorkWithUrls; priority?: boolean }) {
  ...
  const canExpand = description.length > DESCRIPTION_LIMIT;
  const preview = canExpand ? `${description.slice(0, DESCRIPTION_LIMIT).trimEnd()}…` : description;
      ...
      <div className="p-5 sm:p-6">
        ...
        <div className="mt-7 flex items-start justify-between gap-5">
          <div className="min-w-0">
            ...
            {description && <p className="mt-3 max-w-[34ch] text-[12px] leading-5 text-white/72">{expanded ? description : preview}</p>}
            {canExpand && <button type="button" onClick={() => setExpanded((value) => !value)} className="mt-2 text-[10px] uppercase tracking-[0.12em] text-white/90 hover:text-white">{expanded ? 'Свернуть ↑' : 'Читать далее →'}</button>}
          </div>
