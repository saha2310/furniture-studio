...
export const Select = forwardRef<HTMLSelectElement, SelectProps>(({ label, error, id, className = '', children, ...props }, ref) => {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <select ref={ref} id={inputId} className={`h-12 border bg-surface px-4 text-sm text-ink transition-colors focus:border-ink/45 ${error ? 'border-danger' : 'border-ink/15'} ${className}`} aria-invalid={!!error} {...props}>{children}</select>
