...
export const Select = forwardRef<HTMLSelectElement, SelectProps>(({ label, error, id, className = '', children, ...props }, ref) => {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <select ref={ref} id={inputId} className={`h-12 border bg-[#1a1918] px-4 text-sm text-ink transition-colors focus:border-white/45 ${error ? 'border-red-400' : 'border-white/15'} ${className}`} aria-invalid={!!error} {...props}>{children}</select>
