...
export const Textarea = forwardRef<HTMLTextAreaElement, TextareaProps>(({ label, error, id, className = '', ...props }, ref) => {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <textarea ref={ref} id={inputId} className={`min-h-[130px] resize-y border bg-transparent px-4 py-3 text-sm leading-6 text-ink placeholder:text-stone transition-colors focus:border-ink/45 ${error ? 'border-red-400' : 'border-ink/15'} ${className}`} aria-invalid={!!error} aria-describedby={error ? `${inputId}-error` : undefined} {...props} />
