...
export default async function AboutPage() {
  ...
  return (
    <div className="pt-[82px]">
      <section className="container-studio grid min-h-[560px] items-end gap-12 border-b border-white/10 py-20 lg:grid-cols-[.9fr,1.1fr] lg:py-24">
        <div>
        ...
        <div className="relative aspect-[4/3] overflow-hidden bg-white/5">
          {imagePath ? <Image src={workImageUrl(imagePath)} alt="Мастерская мягкой мебели" fill priority sizes="(min-width: 1024px) 55vw, 100vw" className="object-cover" /> : <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_45%,#433f39,#211f1d_50%,#111110)]" />}
        </div>
      </section>
      <section className="border-b border-white/10 bg-[#d8d3ca] text-[#272521]">
        <div className="container-studio grid gap-12 py-20 lg:grid-cols-[.7fr,1.3fr] lg:gap-24 lg:py-24">
          <div><p className="text-[10px] uppercase tracking-[0.2em] text-black/45">о нас</p><p className="mt-8 text-[clamp(4rem,7vw,6rem)] leading-none tracking-[-0.06em]">12<span className="ml-3 text-lg tracking-normal">лет</span></p><p className="mt-3 max-w-[18ch] text-xs leading-5 text-black/50">опыта в производстве мягкой мебели</p></div>
          <div><p className="max-w-[54ch] whitespace-pre-line text-[16px] leading-8 text-black/65">{body}</p><div className="mt-12 flex flex-wrap gap-x-10 gap-y-3 border-t border-black/15 pt-5 text-[11px] uppercase tracking-[0.14em] text-black/45"><span>{settings?.company_name || 'УЮТ'}</span><span>Москва</span><span>Индивидуальное производство</span></div></div>
        </div>
      </section>
    </div>
  );
