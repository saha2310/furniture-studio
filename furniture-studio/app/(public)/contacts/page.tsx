  ...
  return (
    <div className="pt-[82px]">
      <section className="grid min-h-[calc(100vh-82px)] lg:grid-cols-2">
        <div className="relative min-h-[620px] overflow-hidden border-b border-white/10 lg:border-b-0 lg:border-r">
          ...
          ) : <div className="absolute inset-0 bg-[radial-gradient(circle_at_65%_45%,#514a42,#26241f_45%,#111110_75%)]" />}
          <div className="absolute inset-0 bg-black/50" />
          <div className="relative z-10 flex h-full flex-col justify-end p-7 pb-12 sm:p-10 lg:p-14">
            <p className="eyebrow text-white/50">контакты</p>
            <h1 className="mt-5 max-w-[8ch] text-[clamp(3.8rem,7vw,7rem)] leading-[.86] tracking-[-.05em] text-white">Обсудить проект</h1>
            <span className="mt-10 h-px w-12 bg-white/40" />
            <p className="mt-6 max-w-[34ch] text-sm leading-6 text-white/65">Оставьте заявку, и мы свяжемся с вами, чтобы обсудить пространство, форму и материалы.</p>
            {contactLinks.length > 0 && (
              <div className="mt-8">
                <p className="eyebrow text-white/40">мы также на связи в</p>
                <div className="mt-3 flex flex-wrap gap-2">
                  {contactLinks.map((link) => (
                    <a key={link.id} href={link.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 border border-white/20 px-4 py-2.5 text-xs text-white/80 hover:border-white/45 hover:text-white">
        ...
        <div className="flex items-center bg-[#171716] px-7 py-14 sm:px-10 lg:px-14">
