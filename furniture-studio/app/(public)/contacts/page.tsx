      ...
      <section className="grid min-h-[calc(100vh-82px)] lg:grid-cols-2">
        <div className="relative min-h-[620px] overflow-hidden border-b border-ink/10 lg:border-b-0 lg:border-r">
          ...
          <div className="light-image-content relative z-10 flex h-full flex-col justify-end p-7 pb-12 sm:p-10 lg:p-14">
            ...
            <p className="mt-6 max-w-[34ch] text-sm leading-6 text-ink/65">Позвоните или напишите напрямую — обсудим пространство, форму и материалы.</p>
          ...
          <div className="w-full max-w-[560px]">
            <div className="mb-12">
              ...
              <p className="mt-4 text-sm leading-6 text-ink/60">
                Позвоните или напишите — обсудим размеры, форму, цвет и материалы напрямую, без форм и заявок.
              </p>
            </div>

            <div className="flex flex-col gap-3">
              {settings?.phone?.split(',').map((rawPhone) => {
                const phone = rawPhone.trim();
                if (!phone) return null;
                return (
                  <a key={phone} href={formatPhoneForHref(phone)} className="group reference-button w-full justify-between">
              ...
              {contactLinks.map((link) => (
                <a key={link.id} href={link.url} target="_blank" rel="noopener noreferrer" className="group reference-button w-full justify-between">
              ...
              {settings?.email && (
                <a href={`mailto:${settings.email}`} className="group reference-button w-full justify-between">
                  <span>Написать на почту · {settings.email}</span>
                  <span className="text-lg transition-transform duration-300 group-hover:translate-x-1">→</span>
                </a>
              )}
            </div>
