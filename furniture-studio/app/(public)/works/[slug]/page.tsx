...
import { getWorkBySlugWithVariants, getAllWorkSlugs } from '@/lib/queries/works';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const work = await getWorkBySlugWithVariants(params.slug);
  if (!work) return {};
  return {
    title: work.title,
              ...
              <div className="mt-6" role="group" aria-label="Выбор цвета">
                ...
                <div className="flex flex-wrap gap-2.5">
                  {work.colorVariants.map((variant) => (
                    <Link
                      key={variant.id}
                      href={`/works/${variant.slug}`}
                      title={variant.colorName ?? undefined}
                      aria-label={variant.colorName ?? 'Цвет'}
                      aria-pressed={variant.id === work.id}
                      className={`fabric-swatch shrink-0 transition-transform ${
                        variant.id === work.id ? 'scale-110 outline outline-2 outline-offset-2 outline-ink' : 'hover:scale-105'
                      }`}
                      style={{ '--swatch-color': variant.colorHex ?? undefined, width: 32, height: 32 } as React.CSSProperties}
                    />
                  ))}
                </div>
              </div>
