...
export default async function AdminSettingsPage() {
  ...
  return <div className="max-w-5xl"><PageHeader title="Настройки" description="Общие данные, SEO, изображения бренда, меню и способы связи." /><div className="mt-6 space-y-3">
    ...
    <AdminSection title="Изображения сайта" description="Логотип, иконка вкладки и картинка для предпросмотра ссылок."><div className="grid gap-4 lg:grid-cols-3"><AssetUploadForm field="logo_path" label="Логотип" currentPath={settings?.logo_path ?? null} currentOriginalPath={settings?.logo_original_path ?? null} help="Логотип бренда в шапке сайта. Сохраняется отдельно и не перекрашивается." /><AssetUploadForm field="favicon_path" label="Иконка вкладки" currentPath={settings?.favicon_path ?? null} currentOriginalPath={settings?.favicon_original_path ?? null} help="Маленькая иконка вкладки браузера. Рекомендуется квадратное изображение." /><AssetUploadForm field="og_image_path" label="Картинка превью" currentPath={settings?.og_image_path ?? null} currentOriginalPath={settings?.og_image_original_path ?? null} help="Картинка для ссылок в мессенджерах и социальных сетях." /></div></AdminSection>
