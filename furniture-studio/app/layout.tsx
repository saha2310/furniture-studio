...
import { buildBusinessSchema } from '@/lib/seo/business-schema';
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  const businessSchema = buildBusinessSchema(siteUrl);

  return (
    <html lang="ru" className={`${fraunces.variable} ${manrope.variable}`} data-theme="dark">
      <head>
        {/*
          JSON-LD карточки компании для Google/Яндекс (локальный пак,
          сниппет в поиске). Данные — в lib/seo/business-schema.ts, здесь
          только вставка <script>. См. комментарий в том файле про то,
          что и почему в разметке пока приблизительно (адрес, логотип).
        */}
        <script
